        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-8">
            ...
            <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter leading-[1.1]">
              From Thought to <br /> <span className="text-gradient">Viral Output.</span>
            </h2>
            <p className="text-slate-400 text-lg leading-relaxed max-w-lg font-light">
              Our AI doesn't just write text; it performs engagement engineering. Select a platform to witness the neural precision of our generated assets.
            </p>
            
            <div className="flex flex-wrap gap-3">
              {(Object.keys(mockOutputs) as Array<keyof typeof mockOutputs>).map((tab) => (
                <button
                  key={tab}
                ...
                >
                  ...
                  <span className="text-sm">{mockOutputs[tab].name.split(' ')[0]}</span>
                </button>
              ))}
            </div>
          </div>
            ...
            >
              ...
              <div className="flex justify-between items-start mb-10 relative z-10">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                     <p className="text-white font-bold text-lg tracking-tight">Smart Generator</p>
                     <p className="text-primary text-[10px] font-black tracking-[0.2em] uppercase">{mockOutputs[activeTab].tag}</p>
                   </div>
                </div>
                <button 
                  onClick={handleCopy} 
                ...
                >
                  {copied ? <Check className="w-5 h-5 text-green-400" /> : <Copy className="w-5 h-5 group-hover/copy:scale-110 transition-transform" />}
                </button>
              </div>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="mt-6 pt-6 border-t border-white/5 flex justify-between items-center">
                    ...
                    <span className="text-[10px] text-slate-600 font-bold uppercase tracking-widest">Optimized by Neural_V3</span>
                  </div>
                </motion.div>
              </AnimatePresence>
            </motion.div>
