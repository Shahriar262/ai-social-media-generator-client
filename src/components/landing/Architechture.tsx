...
const techStack = [
  { name: "TypeScript", category: "Core Language", icon: <SiTypescript className="w-5 h-5 text-[#3178C6]" /> },
  { name: "Next.js 15", category: "Frontend Framework", icon: <SiNextdotjs className="w-5 h-5 text-white" /> },
  { name: "Gemini 3 Flash", category: "Neural Engine", icon: <Sparkles className="w-5 h-5 text-blue-400" /> },
  { name: "Node.js & Express", category: "Backend API", icon: <SiNodedotjs className="w-5 h-5 text-[#339933]" /> },
  { name: "Railway", category: "Backend Hosting", icon: <SiRailway className="w-5 h-5 text-white" /> },
  { name: "Vercel Edge", category: "Static Hosting", icon: <SiVercel className="w-5 h-5 text-white" /> },
];
      ...
      <div className="max-w-6xl mx-auto px-6 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {techStack.map((tech, i) => (
            <motion.div
              key={i}
              whileHover={{ y: -5, borderColor: "rgba(59, 130, 246, 0.6)", backgroundColor: "rgba(255,255,255,0.1)" }}
